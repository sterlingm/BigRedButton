...
/// </summary>
	public static class StringExtensions
	{
		...
		{
			if (myValue.IsNullOrEmpty() == false) {
				myValue = myValue.Trim();
		...
		{
			if (val.IsNullOrEmpty() == false && (ignoreCase ? val.StartsWithIgnoreCase(prefix) : val.StartsWith(prefix))) {
		...
		{
			if (val.IsNullOrEmpty() == false && (ignoreCase ? val.EndsWithIgnoreCase(suffix) : val.EndsWith(suffix))) {
		...
		{
			if (val.IsNullOrEmpty() || (ignoreCase ? val.EndsWithIgnoreCase(suffix) : val.EndsWith(suffix))) {
				return val;
			}
			return val + suffix;
		}
		...
		{
			if (val.IsNullOrEmpty() || (ignoreCase ? val.StartsWithIgnoreCase(prefix) : val.StartsWith(prefix))) {
				return val;
			}
			return prefix + val;
		}
